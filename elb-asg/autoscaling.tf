...
resource "aws_launch_configuration" "elb-asg" {
  security_groups = [ aws_security_group.elb-asg-security-group.id ]
...
resource "aws_autoscaling_group" "elb-asg" {
  name = "alb-asg"
  min_size = 2
  max_size = 2
  launch_configuration = aws_launch_configuration.elb-asg.name
  
  health_check_type = "ELB"
  health_check_grace_period = 300

  load_balancers = [ aws_elb.elb-asg.name ]
...
resource "aws_autoscaling_notification" "autoscaling_notification" {
  group_names = [ "${aws_autoscaling_group.elb-asg.name}" ]
  topic_arn = aws_sns_topic.elb-asg-sns.arn
